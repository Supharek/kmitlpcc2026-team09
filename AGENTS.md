...
6. **`docs/BRIEF.md` แก้ได้เฉพาะหัวข้อ "เป้าหมายที่เว็บต้องทำให้ได้ (G)"** เท่านั้น
   ทีมปรับ G1–G7 ให้เข้ากับบริษัทของตัวเองได้ (เพิ่ม/แก้ถ้อยคำ/เปลี่ยนระดับ Must-Should-Could)

   > ส่วนที่เหลือ **ห้ามแตะเด็ดขาด** แม้ผู้ใช้จะสั่ง — ให้ทักท้วงแล้วอธิบายว่าทำไม
   > - **T1–T6** เป็นเกณฑ์ที่ทุกทีมต้องมีเหมือนกัน Instructor ใช้ตรวจทั้ง 10 ทีมด้วย checklist เดียว
   > - **N1–N4** เกณฑ์คุณภาพ ใช้ตรวจเหมือนกันทุกทีม
   > - **ส่วนที่ 2 (CI แบรนด์)** และ **ส่วนที่ 3 (Vercel/Supabase)** เป็นข้อกำหนดและข้อเท็จจริงทางเทคนิค ไม่ใช่ความเห็น
...
> ### ⚠️ ทุกทีมใช้ฐานข้อมูลตัวเดียวกัน — ตารางต้องมี prefix เลขทีม
>
> **ชื่อตารางทุกตารางต้องขึ้นต้นด้วย `tNN_`** โดย `NN` คือเลขทีมสองหลักของ repo นี้
> เช่น repo `kmitlpcc2026-team06` ใช้ `t06_contacts`, `t06_services`
>
> - ใช้กับทุกที่: `db/migrations/*.sql`, `docs/DESIGN.md`, และทุก `.from("...")` ในโค้ด
> - **ห้ามสร้าง อ่าน แก้ หรือลบตารางที่ขึ้นต้นด้วยเลขทีมอื่นเด็ดขาด**
> - ถ้าเจอตารางที่ยังไม่มี prefix ในไฟล์ใด ๆ **ให้แก้ทันทีโดยไม่ต้องรอถาม**
>
> Supabase Free จำกัด 2 database ต่อ 1 องค์กร จึงต้องใช้ร่วมกัน — prefix คือสิ่งเดียวที่กันงานทีมอื่นพัง
